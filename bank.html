...
const COLORS = ['#ffd166','#3ec4ff','#6ee7b7','#b388ff']; // Pie chart colors
let lastData = null;
...
async function fetchData(){
  try {
    ...
    if(dataStr === lastData){ document.getElementById('status').textContent='Keine Änderung'; return; }
    lastData = dataStr;
    ...
    document.getElementById('investmentTitle').textContent = `Investitionsaufteilung – ${category}`;
    ...
    pieChart.data.datasets[0].backgroundColor = invLabels.map((_,i)=>COLORS[i % COLORS.length]);
    pieChart.update();
    renderLegend(invItems);

    // ROI chart: combine historical + forecast
    ...
    const histLabels = roiHist.map(p=>p.x);
    const histValues = roiHist.map(p=>p.y);

    const futLabels = roiFut.map(p=>p.x);
    ...
    lineChart.data.datasets = [
      {label:'Historisch', data:histValues.concat(new Array(futValues.length).fill(null)), borderColor:'#3ec4ff', borderWidth:2, fill:false, tension:0.3},
      {label:'Prognose', data:new Array(histValues.length).fill(null).concat(futValues), borderColor:'#ff8fab', borderWidth:2, fill:false, tension:0.3, borderDash:[8,4]}
    ];
